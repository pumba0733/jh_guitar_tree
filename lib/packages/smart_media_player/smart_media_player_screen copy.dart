...
Future<Float32List> decodeWavToFloat32(File file) async {
  ...
  // === WAV Header ===
  final format = data.getUint16(20, Endian.little); // PCM=1, Float=3
  final sampleRate = data.getUint32(24, Endian.little);
  final bitsPerSample = data.getUint16(34, Endian.little);

  if (sampleRate != 44100) {
    debugPrint('[WAV] ⚠️ Non-44100Hz, SoundTouch expects 44100');
  }
  ...
  for (int i = 12; i < bytes.length - 8; i++) {
    if (bytes[i] == 0x64 &&
        ...
        bytes[i + 2] == 0x74 &&
        bytes[i + 3] == 0x61) {
      dataChunkPos = i + 8; // skip "data" + size(4)
      break;
    }
  }
  ...
  final pcmBytes = bytes.sublist(dataChunkPos);
  final pcmData = ByteData.view(pcmBytes.buffer, pcmBytes.offsetInBytes);

  // === CASE 1: 32bit float WAV (format=3, bits=32) ===
  ...
  // === CASE 2: 16bit PCM WAV (format=1, bits=16) ===
  if (format == 1 && bitsPerSample == 16) {
    final sampleCount = pcmBytes.length ~/ 2;
    final out = Float32List(sampleCount);

    for (int i = 0; i < sampleCount; i++) {
      final s = pcmData.getInt16(i * 2, Endian.little);
      out[i] = s / 32768.0;
    }
    return out;
  }

  // === CASE 3: 24bit PCM WAV ===
  if (format == 1 && bitsPerSample == 24) {
    final sampleCount = pcmBytes.length ~/ 3;
    final out = Float32List(sampleCount);

    for (int i = 0; i < sampleCount; i++) {
      final b0 = pcmBytes[i * 3];
      final b1 = pcmBytes[i * 3 + 1];
      final b2 = pcmBytes[i * 3 + 2];
      int v = (b2 << 16) | (b1 << 8) | b0;
      if (v & 0x800000 != 0) v |= 0xFF000000; // sign extend
      out[i] = (v / 8388608.0).clamp(-1.0, 1.0);
    }
    return out;
  }

  throw Exception("Unsupported WAV format: format=$format bits=$bitsPerSample");
}
...
class _SmartMediaPlayerScreenState extends State<SmartMediaPlayerScreen>
  ...
  bool _isWavFile(String path) {
    final ext = p.extension(path).toLowerCase().trim();
  ...
  void _startSyncTimer() {
    _syncTimer?.cancel();
    _syncTimer = Timer.periodic(const Duration(milliseconds: 80), (_) async {
      if (!mounted || !_isVideo) return;

      // 1) 오디오 위치(SoundTouch)
      final t = ac.SoundTouchAudioChain.instance.playbackTime;
      final audioPos = Duration(milliseconds: (t * 1000).round());
      await _player.seek(audioPos);
  ...
  void initState() {
    ...
    _wf.viewStart.addListener(() {
      if (mounted) setState(() {});
    });
    _wf.viewWidth.addListener(() {
      if (mounted) setState(() {});
    });
    ...
    // 🔧 비동기 초기화는 분리
    _initAsync();
    _wf.attachTo(ac.SoundTouchAudioChain.instance);
  ...
  Future<Duration> getWavDuration(File file) async {
    ...
    // 'data' 청크 위치 찾기 (헤더 내 위치가 일정하지 않음)
    int dataChunkPos = -1;
    for (int i = 12; i < bytes.length - 8; i++) {
      if (bytes[i] == 0x64 &&
          bytes[i + 1] == 0x61 &&
    ...
    if (dataChunkPos == -1) return Duration.zero;

    final dataSize = data.getUint32(dataChunkPos + 4, Endian.little);

    final totalSamples = (dataSize * 8) ~/ (bitsPerSample * numChannels);
    final durationSec = totalSamples / sampleRate;

    return Duration(milliseconds: (durationSec * 1000).round());
  }
  ...
  Future<void> _openMedia() async {
    ...
    // 1) always decode audio track to float32 PCM
    debugPrint('[SMP] 🎧 Decoding audio track...');
    final pcm = await AudioDecoder.decodeToFloat32(path);

    // 2) create PCM stream
    final chunkSize = 4096 * 2;
    final chunks = <Float32List>[];
    for (int i = 0; i < pcm.length; i += chunkSize) {
      final end = math.min(i + chunkSize, pcm.length);
      chunks.add(pcm.sublist(i, end));
    }
    ...
    Future(() async {
      for (final c in chunks) {
        controller.add(c);
        await Future.delayed(Duration.zero); // 이벤트 루프 yield
      }
      await controller.close();
    });

    await _chain.start(pcmStream: controller.stream);
    ...
    if (isVideo) {
      ...
      debugPrint('[SMP] 🎬 Video + external PCM mode');
    } else {
      debugPrint('[SMP] 🎵 Audio-only mode (PCM feed)');
    }
    _startSyncTimer();
    // 4) duration 계산
    final channels = 2;
    final sampleRate = 44100;
    final totalSamples = pcm.length / channels;
    final durSec = totalSamples / sampleRate;

    setState(() {
      _duration = Duration(milliseconds: (durSec * 1000).round());
    });

    debugPrint('[SMP] ⏱ duration=$_duration');
  }
  ...
  Future<void> _applyAudioChain() async {
    ...
    if (!chain.isStarted) {
      ...
      await _chain.start(pcmStream: const Stream.empty());
    }
  ...
  Future<void> _applyAudioChainDebounced() async {
   ...
   _applyDebounce = Timer(const Duration(milliseconds: 250), () async {
...
// 현장 최적화 4종 (1/2/4/8마디)
  ...
  Future<void> _spacePlayBehavior() async {
    ...
    debugPrint('[SMP] ▶ SPACE pressed');

    // === 1) mpv 영상/오디오 싱크 먼저 세팅 ===
    if (_isVideo) {
      await _player.seek(_startCue);
      await _player.play();
    } else {
      await _player.seek(_startCue);
      await _player.pause(); // audio-only(video 없음)
    }

    // === 2) PCM 디코드 ===
    final Float32List pcm = _isWavFile(widget.mediaPath)
        ? await decodeWavToFloat32(file)
        : await AudioDecoder.decodeToFloat32(widget.mediaPath);
    ...
    final chunks = <Float32List>[];
    for (int i = 0; i < pcm.length; i += chunkSize) {
      final end = (i + chunkSize).clamp(0, pcm.length);
      chunks.add(pcm.sublist(i, end));
    }
    ...
    debugPrint('[SMP] ▶ SPACE feed started');
  }
  ...
  Future<void> _setPitch(int semis) async {
    ...
    _chain.setPitch(_pitchSemi.toDouble());
    _debouncedSave();
  }
  ...
  Future<void> _setVolume(int v) async {
    ...
    _chain.setVolume(_volume / 100.0);
    _debouncedSave();
  }
